...
contract FlightSuretyData {
    ...
    // address payable public data contract address
    ...
    */
    ...
    public
    {
        ...
        // First arline is registered when contract is deployed
        airline[msg.sender] = Airline({
   ...
   * @dev Modifier that requires the calling App contract has been authorized
    ...
    */
    ...
    requireIsOperational
    {
        for (uint256 n = 0; c < passengerAddresses.length; n++) {
            if (passengers[passengerAddresses[n]].boughtFlight[flightCode] != 0) {
                uint256 savedCredit = passengers[passengerAddresses[n]].credit;
                uint256 payedPrice = passengers[passengerAddresses[c]].boughtFlight[flightCode];
                passengers[passengerAddresses[n]].boughtFlight[flightCode] = 0;
                passengers[passengerAddresses[n]].credit = savedCredit + payedPrice + payedPrice.div(2);
            }
        }
    }
